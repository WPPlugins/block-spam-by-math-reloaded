...
define('BSBM_VERSION', '2.2.3');	// Current version of the Plugin
...
function bsbm_add_hidden_fields($errors = '') {
	...
	<input type="text" tabindex="'. $options['bsbm_tabindex'] .'" name="mathvalue2" value="" />
		...
		<input type="text" name="mathvalue0" value="'. $mathvalue0 .'" />
		<input type="text" name="mathvalue1" value="'. $mathvalue1 .'" />
		</div>
	</div>
	<div class="notice">';
...
function bsbm_check_hidden_fields() {
	...
	elseif ( ( $val0 == '' ) || ( $val1 == '' ) || ( intval($val2) != (intval($val0) + intval($val1)) ) ) { return $answer=2; }
...
function bsbm_activate() {
	...
	$default_options = array( 
		'bsbm_empty_error' => BSBM_ANSWER_ERROR,
		'bsbm_answer_error' => BSBM_EMPTY_ERROR,
...
function bsbm_postbox_sponsor() {
	global $bsbm_fullpath;
	$output .= '<p><img style="align:left;" src="'. $bsbm_fullpath .'/images/thesis-260x125.png" width="200" title="Run a Killer Website with the Thesis WordPress Theme" alt="Run a Killer Website with the Thesis WordPress Theme" /></p>';
	$output .= '<p>Thesis 1.x changed the world of WordPress themes. Instead of focusing on design, like countless other themes, Thesis 1.x focused on building a rock-solid framework.</p>';

	$output .= '<p>It wasn\'t about the design...</p>';	
		
	$output .= '<p>...It was about a solid foundation that gave you "point-and-click" design
controls, lightning-fast load times, and SUPERB SEO functionality.</p>';	
	$output .= '<p>However, the time is coming, and Thesis is about to change the theme space once again. Thesis 2.0 is currently in development, and you should expect to hear more soon.</p>';
	$output .= '<p>We\'ve been keeping Thesis 2.0 a closely guarded secret--similar to how Apple keeps their products a secret--because we have some truly remarkable, game-changing features planned.</p>';	
	$output .= '<p>Talk soon,<br />Chris Pearson, and the DIYthemes Team</p>';

	$output .= '<p align="center"><a href="http://www.shareasale.com/r.cfm?b=198392&u=430275&m=24570&urllink=&afftrack=" target="_blank"><img style="align:left;" src="'. $bsbm_fullpath .'/images/thesis-getit.jpg" width="200" title="" alt="" /></a></p>';	
